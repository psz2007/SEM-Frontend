...
1. 选择 Terminal 界面，选择合适的端口（一般是 COM3），点击连接，观察右侧提示，若显示成功则当前步骤完成；
...
4. 切换至 Picture 界面，使用 Refresh 和 Update 键更新输出界面（十六进制格式和图片格式），Auto Refresh 每 0.5s 更新一次。（Superpose 功能未经测试，慎用）

   当界面不再更新，进度条至 100% 时扫描完成，使用 Export 键导出图片，默认存在用户的 Download 目录下，文件名为 `pic_<时间戳>.png`。

   使用滑条可调整图像亮度，将区间内的亮度（超出区间的部分向上下界对齐）映射至 0~255。（换言之，调大下限会使图像变暗，调小下限会使图像变亮）。
